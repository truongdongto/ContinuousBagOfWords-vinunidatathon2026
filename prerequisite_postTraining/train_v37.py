...
VARIANT = 'pattern'  # v37 builds on v33_pattern (verified best LB)

SCRIPT_DIR = Path(__file__).resolve().parent
CKPT_PATH = SCRIPT_DIR / f'ml_v37_{VARIANT}_ckpt.npz'
OUT_PATH  = SCRIPT_DIR / f'ml_v37_{VARIANT}_preds.npz'

print(f"=== Training v37_{VARIANT} (v3 promo + 5 curated customer/product features) ===", flush=True)
t_start = time.time()

# ============ Load data ============
df = pd.read_pickle(SCRIPT_DIR / f'enriched_features_v6_{VARIANT}.pkl').sort_values('Date').reset_index(drop=True)
