...
VARIANT = 'pattern'  # v45 builds on v33_pattern (verified best LB)

SCRIPT_DIR = Path(__file__).resolve().parent
CKPT_PATH = SCRIPT_DIR / f'ml_v45_{VARIANT}_ckpt.npz'
OUT_PATH  = SCRIPT_DIR / f'ml_v45_{VARIANT}_preds.npz'

print(f"=== Training v45_{VARIANT} (v3 promo + v8 + 6 web/review features) ===", flush=True)
t_start = time.time()

# ============ Load data ============
df = pd.read_pickle(SCRIPT_DIR / f'enriched_features_v10_{VARIANT}.pkl').sort_values('Date').reset_index(drop=True)
